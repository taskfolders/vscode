...
import { parseSidLinks, parseUidLinks } from './parseUidLinks'
    ...
    vscode.languages.registerDocumentLinkProvider(
      ...
      },
      {
        provideDocumentLinks(
          ...
          let allSids = parseSidLinks(text)
          for (let item of allSids) {
            //console.log('Found uuid match', match)
            const startPos = document.positionAt(item.index + 0) // Add 2 to skip the [[
            const endPos = document.positionAt(item.index + item.sid.length + 1) // Subtract 2 to skip the ]]
